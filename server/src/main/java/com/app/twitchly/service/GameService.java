...
import com.app.twitchly.entity.response.Game;
...
public class GameService {
    ...
    // define url information for search
    // by visiting the url, we can directly go to the online stream page
    ...
    private List<Item> searchStreams(String gameId, int limit) throws TwitchException {
        // using getItemList method to convert json object of streams to List<Item>
    ...
    // based on different types, transfer request to each particular service method
    public List<Item> searchByType(String gameId, ItemType type, int limit) throws TwitchException {
        ...
        // some List<Item> items generated by search method do not have gameID, so if we want to keep our
        // data consistent, we need to update gameID
        for (Item item : items) {
